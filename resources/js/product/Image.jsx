...
export function MainImage({ src, alt }) {
  ...
  return (
    <div className="flex items-center justify-center mt-5" onClick={() => modals.open({
      centered: true,
      padding: "0",
      size: "70%",
      withCloseButton: false,
      children: (
        <img src={src} alt={alt} className="w-full" style={{ objectFit: 'contain' }} />
      )
    })}>
      <img src={src} alt={alt} className="w-full max-h-[500px] z-[1000]" style={{ objectFit: 'contain' }} />
    </div>
  )
}
...
export default function Gallery({ images }) {
  ...
  return (
    <Flex className="max-h-[500px] flex-col md:flex-row">
